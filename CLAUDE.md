...
## 11. Production wiring discipline

A new production symbol `X` (class, top-level function, Protocol, or registry) is not considered shipped until both:

1. **Contract gate** — at least one anchor / unit test in `tests/` locks the documented behavior of `X`.
2. **Caller gate** — at least one non-test module under `backend/src/qts/`, `scripts/`, or `examples/` imports and actually invokes `X` on a code path reachable from a real entrypoint (API route, CLI script, actor handler, or backtest engine).

Exceptions and refinements:

- **Protocols** — the Protocol itself is not invoked; instead, at least one production-side concrete implementation must exist (test fakes do not count). Protocols added only to narrow a single internal type annotation are forbidden by §5 and should be replaced with the concrete type.
- **Registries** — registering zero entries in production code fails the caller gate. Ship at least one real entry that exercises the registry contract end-to-end (e.g. a v0 → v1 schema migration, even if no payload shape change is required yet).
- **Pure SDK exports** for user strategies — exporting a type for third-party consumption satisfies the caller gate when at least one example strategy under `examples/strategies/` uses it.
- **Documented deferral** — when a follow-up PR will land the caller, add the fully-qualified symbol to `docs/plan/wiring_deferrals.md` with a one-line rationale and the deferral category (documented deferral, library API, framework integration, or module-internal helper). The deferral must be removed in the same PR that adds the caller.

Why: shipped-but-unwired production paths have appeared in OPT-15, 17, 25, 26, and 29 — each required a separate follow-up PR to actually take effect. Closing the gap at landing time is cheaper than tracking wiring debt across releases.

Enforcement: `qts.quality.rules.caller_presence.CallerPresenceRule` (registered in the default `GuardrailSuite`) fails `make guardrails` when a baseline-listed public symbol has zero non-test callers under `backend/src/qts/`, `scripts/`, or `examples/`. Auto-exempt: Protocol subclasses, Enum subclasses, exception subclasses, and value objects referenced as return / attribute annotations in their defining module. Explicit deferrals live in `docs/plan/wiring_deferrals.md`.

## 12. Knowledge graph / MCP usage
