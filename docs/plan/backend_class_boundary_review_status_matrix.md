...
| BacktestEngine | 387 | Backtest engine | High | Retain | Engine orchestrates the actor loop over prepared replay inputs; risk/margin policy (`BacktestRiskPolicyFactory`), dataset manifest/provenance (`BacktestDatasetManifestBuilder`), and runtime-actor public surface are extracted (QTS-FINAL-002). Remaining extraction: artifact service + topology manifest + engine assembler. | `BacktestEngineCohesionRule`; `PublicSurfaceRule`; `tests/unit/scripts/test_verify_guardrails.py` | Complete |
