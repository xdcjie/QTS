...
| IbkrCallbackNormalizer | 533 | Execution adapters | High | Retain with split target | Keep callback idempotency and normalization centralized; split only when independent callback families need separate public owners. | `tests/unit/execution/test_ibkr_callback_idempotency.py`; `docs/plan/qts_final_platform_freeze_review_status_matrix.md` | Complete |
...
| BrokerRuntimeStartupChecklist | 330 | Runtime startup | High | Retain | Keep startup readiness facts and live-capital gate evidence in one startup checklist boundary. | `tests/unit/runtime/test_live_startup_guard.py`; `docs/plan/qts_final_platform_freeze_review_status_matrix.md` | Complete |
