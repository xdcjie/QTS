...
| --- | --- | --- | --- | --- |
| OPT-01 Strongly-type Strategy SDK callbacks | DONE / Focused gates passed | `qts.strategy_sdk.events` owns public `TimerEvent`, `OrderUpdate`, and `Fill`; `Strategy` callbacks use `StrategyContext`, `Bar`, `Tick`, and SDK events; VWAP example has no `Any`; `docs/strategy_sdk/strategy_api.md` documents callback payloads. | None for focused SDK typing gates. | `test_strategy_callback_signatures_use_public_sdk_types` and `test_vwap_pullback_example_does_not_use_any` failed before the SDK event module and typed signatures existed. |
