"""Instrument and roll-resolution helpers for runtime execution."""
...
class RuntimeInstrumentContext:
    """Resolve runtime instrument IDs, roll targets, and instrument metadata."""
    ...
    def instrument_registry(self) -> InstrumentRegistry:
        ...
        if not self._registry_bars:
            raise RuntimeError(
                "instrument_registry is required when bars are streamed from a one-pass iterable"
            )
...
__all__ = ["RuntimeInstrumentContext"]
