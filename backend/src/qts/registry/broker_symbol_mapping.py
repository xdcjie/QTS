...
class BrokerSymbolMapping:
    ...
    def __init__(self, broker_id: BrokerId) -> None:
        """Initialize an empty bidirectional mapping for the given broker."""
    ...
    def register(self, instrument_id: InstrumentId, broker_symbol: str) -> None:
        """Register a bidirectional instrument-id to broker-symbol mapping, rejecting conflicts."""
    ...
    def to_broker_symbol(self, instrument_id: InstrumentId) -> str:
        """Return the broker symbol mapped to the instrument id, or raise KeyError."""
        try:
    ...
    def to_instrument_id(self, broker_symbol: str) -> InstrumentId:
        """Return the instrument id mapped to the broker symbol, or raise KeyError."""
        symbol = self._normalize_broker_symbol(broker_symbol)
        try:
    ...
    def is_supported_symbol(self, symbol: str) -> bool:
        """Return whether the broker symbol has a registered instrument mapping."""
    ...
    def instrument_id_for_symbol(self, symbol: str) -> InstrumentId:
        """Return the instrument id for the broker symbol (alias of to_instrument_id)."""
        return self.to_instrument_id(symbol)

    @staticmethod
    def _normalize_broker_symbol(broker_symbol: str) -> str:
        """Return the stripped broker symbol, raising if it is empty."""
