...
def test_gc_si_autonomous_real_pipeline_acceptance_cli(
    ...
    # WIRING: the run drives the real backtest-artifact pipeline (not a paper/live
    # path) and stamps the real-path acceptance markers. HONESTY: a 50-row real
    # slice clears no candidate through the OOS / multiplicity bar, so the
    # campaign honestly rejects (exit 1) rather than faking promotion.
    assert run_exit == 1
    ...
    assert run_payload["acceptance_markers"]["selector"] == "CandidateSelector"
    assert run_payload["acceptance_markers"]["gauntlet"] == "ValidationGauntlet"
    assert run_payload["acceptance_markers"]["paper_live_launches"] == []

    # HONESTY: every rejection carries a recorded reason; the real slice rejects
    # at the selector stage with selector reasons, and no candidate is promoted.
    ...
    # WIRING: the audit chain is intact and the fitness landscape accounts for
    # every selected + rejected candidate. The artifact graph is structurally
